...
class Flowmatching(nn.Module):
    ...
    def __init__(self, cfg: FMConfig):
        ...
        self.timesteps_per_stage = torch.from_numpy(timesteps[:-1]).float() # [1000,999,...,1]
        self.register_buffer('_tps', self.timesteps_per_stage, persistent=False)
        self.stage_sigmas = np.linspace(1, 0, self.max_steps + 1,) # [1,0.999,...,0.001,0]
        self.sigmas_per_stage = torch.from_numpy(self.stage_sigmas[:-1]).float() # [1,0.999,...,0.001]
    ...
    def sample_euler(
        ...
        """
        with torch.autocast(_autocast_device(), dtype=torch.bfloat16, enabled=_autocast_ok()):
